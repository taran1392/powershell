...
#$command.CommandText
    ...
    $command.CommandText = @" 
merge $table as T 
using (select tenant,logid,createdon,auditdata,operation,userid,recordtype from $stagingtable group by tenant,logid,createdon,auditdata,recordtype,operation,userid) as S
