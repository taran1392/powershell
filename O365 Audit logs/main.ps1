  ...
  param($user,$password,$tenant)
        ...
        $command_script_block={
            ...
            do{
               ...
               $hasNext= Search-UnifiedAuditLog -EndDate $enddate  -RecordType powerbi -StartDate $startdate -SessionId $sessionid -ResultSize $resultsize
                $logs+=$hasNext
        }while($hasNext)
...
$logs=Invoke-Command -ScriptBlock $command_script_block -ArgumentList $startdate,$enddate -Session $Session
#$logs|ft recordtype,creationdate,userids,operations,auditdata
