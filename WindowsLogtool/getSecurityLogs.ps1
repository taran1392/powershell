...
<#$getEventInput = StackPanel -ControlName 'Get-EventLogsSinceDate' {            
    New-Label -VisualStyle 'MediumText' "Get Security Logs Since..."            
    New-ComboBox -IsEditable:$false -SelectedIndex 0 -Name LogName @("Application", "Security", "System", "Setup") -Visibility Hidden           
...
Clear-Host
   ...
   $logs|Export-Csv -NoTypeInformation -Append "$pwdir\securityLogs.txt"
